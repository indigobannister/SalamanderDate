...
graph_2 <- ggplot() +
  geom_density_ridges(data = salamander_weight_2017, 
                aes(x = weight,
                    y = channel_class))
...
graph_2c <- ggplot() +
geom_beeswarm(data = salamander_weight_2017, 
              aes(y = weight,
                  x = channel_class))
...
# So many outliers! Let's make sure to do something with medians
...
salamander_weight_summary <- salamander_weight_2017 %>% 
  group_by(channel_class) %>% 
  summarize(
    "Mean weight (g)" = mean(weight, na.rm = TRUE),
    "Median weight" = median(weight, na.rm = TRUE),
    "Standard deviation" = sd(weight, na.rm = TRUE),
    "Sample size" = n(),
    "Standard error" = sd(weight, na.rm = TRUE) / sqrt(n()),
...
- These findings may suggest that Pacific giant salamanders are resilient to perturbation.
